...
// Project headers
#include <pins.h>          // bbb_pin_t


/**
 * @def PINS_TABLE_SIZE
 * @brief Size of the pins table
 */
#define PINS_TABLE_SIZE 96

bbb_pin_t     pins_table[PINS_TABLE_SIZE] =
{
    {"USR0", "USR0", 53, -1, -1, 1},
    {"USR1", "USR1", 54, -1, -1, 1},
    {"USR2", "USR2", 55, -1, -1, 1},
...
int bbb_get_gpio_by_name(const char *name)
{
    ...
    {
        if ( strcmp(pins_table[i].name, name) == 0 )
        {
            return (pins_table[i].gpio);
        }
    }
...
int bbb_get_gpio_by_key(const char *key)
{
    ...
    for ( i = 0; i < PINS_TABLE_SIZE; ++i )
    {
        if ( strcmp(pins_table[i].key, key) == 0 )
        {
            return (pins_table[i].gpio);
        }
    }
...
unsigned int bbb_is_pin_pwm(const unsigned int gpio)
{
    ...
    for ( i = 0; i < PINS_TABLE_SIZE; ++i )
    {
        ...
        {
            return ( (pins_table[i].pwm_mux_mode != -1) ? 1 : 0);
        }
    }
...
int bbb_is_pin_allocated_by_default(const unsigned int gpio)
{
    ...
    for ( i = 0; i < PINS_TABLE_SIZE; ++i )
    {
        if ( pins_table[i].gpio == gpio )
        {
            return (pins_table[i].is_allocated_by_default);
        }
    }
